...
def count_identity_patterns(node):
    ...
    if node.node_type == NodeType.IDENTITY_PATTERN:
        count += 1
        print(f"      Found IDENTITY_PATTERN node: {node.value}")
        ...
        if node.value == "+" and len(node.children) >= 2:
            ...
            right = node.children[1]
            if right.node_type == NodeType.LITERAL and right.value == "0":
                count += 1
                print(f"      Found - identity: _ - {right.value}")
...
print(f"\nTotal identity patterns in file: {total_identity_count}")
...
for expr in test_expressions:
    ...
    print(ast.to_string("  "))
    identity_count = count_identity_patterns(ast)
