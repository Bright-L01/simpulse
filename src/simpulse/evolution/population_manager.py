...
class PopulationManager:
    ...
    def __init__(
        self, config: EvolutionConfig, claude_client: Optional[ClaudeCodeClient] = None
    ):
    ...
    async def initialize_population(
        self,
        rules: List[SimpRule],
        claude_suggestions: Optional[List[MutationSuggestion]] = None,
    ) -> Population:
        ...
        baseline = Candidate(
            id="baseline", mutations=[], generation=0, creation_method="baseline"
        )
        ...
        if claude_suggestions:
            for i, suggestion in enumerate(
                claude_suggestions[: self.config.population_size // 3]
            ):
                candidate = Candidate(
                    id=f"claude_{i}",
        ...
        logger.info(
            f"Created initial population: {claude_candidates} Claude-based, "
    ...
    def tournament_selection(
        self, population: Population, tournament_size: Optional[int] = None
    ) -> Candidate:
        ...
        tournament = random.sample(
            valid_candidates, min(tournament_size, len(valid_candidates))
        )
    ...
    async def _two_point_crossover(
        self, parent1: Candidate, parent2: Candidate
    ) -> Candidate:
    ...
    async def _random_mutation(
        self, candidate: Candidate, rules: List[SimpRule]
    ) -> Candidate:
        ...
        if operation == "add" and len(mutated.mutations) < 5:
            ...
            new_mutations = await self._generate_random_mutations(
                rules, max_mutations=1
            )
    ...
    async def _adaptive_mutation(
        ...
        if self.diversity_history:
            ...
            if recent_diversity < self.config.diversity_threshold:
                ...
                min(self.current_mutation_rate * 1.5, 0.8)
            else:
                # High diversity, maintain or reduce mutation rate
                max(self.current_mutation_rate * 0.9, 0.05)
        else:
            pass

        return await self._random_mutation(candidate, rules)

    async def _claude_guided_mutation(
        self, candidate: Candidate, rules: List[SimpRule]
    ) -> Candidate:
    ...
    async def _generate_random_mutations(
        self, rules: List[SimpRule], max_mutations: int = 3
    ) -> List:
    ...
    def update_population(
        self, population: Population, offspring: Population
    ) -> Population:
        ...
        if self.config.adaptive_mutation:
            if self.stagnation_counter > 5:
                ...
                self.current_mutation_rate = max(
                    self.current_mutation_rate * 0.95, 0.05
                )
    ...
    def get_population_statistics(self, population: Population) -> PopulationStatistics:
        ...
        for candidate in valid_candidates:
            for mutation in candidate.mutations:
                ...
                mutation_counts[str(mut_type)] = (
                    mutation_counts.get(str(mut_type), 0) + 1
                )
