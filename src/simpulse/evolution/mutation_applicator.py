...
from .models import SimpRule, MutationSuggestion, MutationType, SimpPriority, SimpDirection
...
class MutationApplicator:
    ...
    def _compile_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            ...
            'full_attr': re.compile(
                r'@\[simp(?:\s+(high|low|\d+))?(?:\s*([↓←]))?\]',
                re.MULTILINE
            ),
            # Match just the simp part for replacement
            'simp_content': re.compile(
                r'simp(?:\s+(high|low|\d+))?(?:\s*([↓←]))?'
            ),
    ...
    async def apply_mutation(self, 
                           rule: SimpRule, 
                           suggestion: MutationSuggestion) -> AppliedMutation:
    ...
    def _apply_mutation_to_content(self, 
                                 content: str, 
                                 rule: SimpRule, 
                                 suggestion: MutationSuggestion) -> Tuple[str, Tuple[int, int]]:
        ...
        if suggestion.mutation_type == MutationType.PRIORITY_CHANGE:
            lines[attr_line] = self._apply_priority_change(
    ...
    def _find_simp_attribute_line(self, 
                                lines: List[str], 
                                rule_line: int, 
                                rule_name: str) -> Optional[int]:
    ...
    def _apply_priority_change(self, line: str, suggestion: MutationSuggestion) -> str:
        ...
        modified = self._simp_attr_patterns['simp_content'].sub(
            replace_simp, line
        )
    ...
    async def apply_mutation_set(self, 
                               mutations: List[MutationSuggestion],
                               rules: List[SimpRule],
                               workspace: Path) -> MutationSet:
