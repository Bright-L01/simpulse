...
from .models import SimpRule, ModuleRules, SimpPriority, SimpDirection, SourceLocation
...
class RuleExtractor:
    ...
    def _find_declaration_after_attribute(self, 
                                        lines: List[str], 
                                        attr_line: int, 
                                        attr_info: Dict) -> Optional[Dict]:
            ...
            if in_declaration:
                ...
                if (brace_count == 0 and paren_count == 0 and 
                    (stripped.endswith(':=') or 
                     stripped.startswith(('theorem', 'lemma', 'def', 'instance', '@[')) or
                     stripped == '' and i > start_line)):
                    break
                    
        return '\n'.join(decl_lines)
        
    def _create_simp_rule(self, 
                         declaration: Dict, 
                         attr_info: Dict, 
                         file_path: Path, 
                         line_num: int) -> Optional[SimpRule]:
        ...
        try:
            ...
            logger.warning(f"Failed to create simp rule from {declaration.get('name', 'unknown')}: {e}")
            return None
            
    def _parse_declaration_components(self, declaration: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    def _extract_conditions(self, declaration: str) -> List[str]:
        ...
        if ':' in declaration:
            type_part = declaration.split(':')[1].split(':=')[0]
    ...
    def find_rules_by_pattern(self, 
                             rules: List[SimpRule], 
                             pattern: str) -> List[SimpRule]:
        ...
        for rule in rules:
            if (rule.pattern and pattern_re.search(rule.pattern)) or \
               (rule.name and pattern_re.search(rule.name)) or \
               pattern_re.search(rule.declaration):
