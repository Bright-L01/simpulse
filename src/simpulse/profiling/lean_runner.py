...
from ..security.validators import is_safe_path, validate_file_path, validate_command_args
...
class LeanRunner:
    ...
    async def profile_module(self,
                            module_name: str,
                            options: Optional[Dict[str, Any]] = None) -> Tuple[LeanResult, Optional[Dict]]:
        ...
        try:
            ...
            parsed_data = None
            if options.get('parse_output', True) and result.output_files and result.output_files.get('profile'):
    ...
    def get_trace_command(self,
                         ...
                         trace_flags: Optional[List[str]] = None) -> List[str]:
    ...
    async def get_simp_diagnostics(self,
                                  file_path: Path,
                                  timeout: float = 300.0) -> Tuple[LeanResult, Optional[Dict]]:
