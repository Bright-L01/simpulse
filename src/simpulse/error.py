...
class OptimizationError(SimpulseError):
    ...
    def __init__(self, message: str, file: Optional[Path] = None, details: Optional[str] = None):
        self.file = file
        if file:
...
class FileError(SimpulseError):
    ...
    def __init__(self, message: str, file: Path, details: Optional[str] = None):
        self.file = file
...
def handle_error(error: Exception, file: Optional[Path] = None, debug: bool = False) -> str:
...
def safe_file_read(file_path: Path, debug: bool = False) -> Optional[str]:
...
def check_memory_usage(operation: str = None) -> None:
