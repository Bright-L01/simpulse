...
def print_warning(message: str, suggestion: str | None = None):
...
def get_error_suggestion(error: Exception, context: str | None = None) -> str:
    ...
    try:
        ...
        if optimized > 0:
            ...
            if results["changes"] and not QUIET_MODE:
                console.print()
                table = Table(title="✨ Optimization Results", title_style="bold green")
                ...
                for change in results["changes"][:10]:
                    ...
                    table.add_row(
                        str(change["rule_name"]), str(old_priority), str(new_priority), impact
                    )
    ...
    try:
        ...
        if results["changes"] and not QUIET_MODE:
            ...
            for change in results["changes"][:5]:
                ...
                table.add_row(
                    str(change["rule_name"]), str(old_priority), str(change["reason"]), impact
                )
...
def guarantee(project_path, save_prediction):
    ...
    try:
        ...
        error_msg = handle_error(e, debug=DEBUG_MODE)
        suggestion = get_error_suggestion(e, "guarantee")
        print_error(f"Performance guarantee failed: {error_msg}", suggestion)
        sys.exit(1)
...
def main():
    ...
    if "--health" in sys.argv:
        try:
            ...
            console_health.print("  - Optimizer: OK")
            console_health.print("  - File processing: OK")
