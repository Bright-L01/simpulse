...
class DiagnosticAnalysis:
    ...
    def get_most_used_theorems(self, limit: int = 10) -> list[SimpTheoremUsage]:
        ...
        return sorted(self.simp_theorems.values(), key=lambda x: x.used_count, reverse=True)[:limit]
...
class DiagnosticParser:
    ...
    def __init__(self):
        ...
        self.simp_used_header = re.compile(r"\[simp\] used theorems \(max: (\d+), num: (\d+)\):")

        # Pattern for simp theorem tries: "[simp] tried theorems (max: 251, num: 2):"
        self.simp_tried_header = re.compile(r"\[simp\] tried theorems \(max: (\d+), num: (\d+)\):")
    ...
    def parse_diagnostic_output(self, output: str) -> DiagnosticAnalysis:
        ...
        analysis.inefficient_theorems = [t.name for t in analysis.get_least_efficient_theorems()]
                ...
                while i < len(lines) and lines[i].strip() and not self._is_new_section(lines[i]):
                    ...
                    for match in self.theorem_usage.finditer(usage_line):
                        ...
                        if theorem_name not in analysis.simp_theorems:
                            analysis.simp_theorems[theorem_name] = SimpTheoremUsage(
                                name=theorem_name
                            )
...
class DiagnosticCollector:
    ...
    def collect_diagnostics(self, files: list[Path] | None = None) -> DiagnosticAnalysis:
...
"""
    ...
    for theorem in analysis.get_most_used_theorems():
        print(
            f"  {theorem.name}: used {theorem.used_count}, tried {theorem.tried_count}, success rate {theorem.success_rate:.2%}"
        )
