...
class CodeRefactor:
    ...
    def _find_dead_code(self, tree: ast.AST, file_path: Path, content: str) -> List[RefactoringSuggestion]:
    ...
    def _find_duplicate_constants(self, tree: ast.AST, file_path: Path, content: str) -> List[RefactoringSuggestion]:
    ...
    def _find_complex_conditionals(self, tree: ast.AST, file_path: Path, content: str) -> List[RefactoringSuggestion]:
    ...
    def _find_unused_imports(self, tree: ast.AST, file_path: Path, content: str) -> List[RefactoringSuggestion]:
        """Find unused imports.
    ...
    def _find_long_functions(self, tree: ast.AST, file_path: Path, content: str) -> List[RefactoringSuggestion]:
            ...
            def visit_FunctionDef(self, node):
                ...
                if node.body:
                    ...
                    if func_length > 50:  # Functions longer than 50 lines
                        lines = content.split('\n')
                        func_preview = '\n'.join(lines[start_line-1:min(start_line+4, end_line)])
    ...
    def analyze_project(self, project_path: Path, 
        ...
        total_lines_to_remove = sum(
            s.estimated_impact.get("lines_removed", 0)
