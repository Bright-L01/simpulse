...
class ClaudeCodeClient:
    ...
    def _validate_claude_installation(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                [self.command, "--version"], capture_output=True, text=True, timeout=10
            )
        ...
        try:
            for attempt in range(self.retry_attempts):
                try:
                    ...
                    logger.debug(
                        f"Executing Claude Code CLI (attempt {attempt + 1}): {' '.join(cmd)}"
                    )
                    ...
                    logger.error(
                        f"Error executing Claude Code CLI (attempt {attempt + 1}): {e}"
                    )
    ...
    def _build_full_prompt(
        self, prompt: str, project_context: Optional[str] = None
    ) -> str:
        """Build complete prompt with context.
    ...
    async def suggest_mutations(
        ...
        prompt = prompt_builder.build_mutation_prompt(
            profile_data=profile_data, rules=rules, top_k=top_k, context=context
        )
