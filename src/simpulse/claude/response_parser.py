...
class ResponseParser:
    ...
    def __init__(self):
        ...
        self.suggestion_patterns = {
            ...
            "mutation_type": re.compile(
                r"(?:Mutation Type|Type):\s*([^\n]+)", re.IGNORECASE
            ),
            "description": re.compile(
                r"(?:Description|Change):\s*([^\n]+)", re.IGNORECASE
            ),
    ...
    def _parse_text_suggestion(self, section_text: str) -> Optional[MutationSuggestion]:
        ...
        try:
            ...
            mutation_type_str = self._extract_field(
                section_text, "mutation_type", "unknown"
            )
            ...
            prerequisites = self._extract_list_field(
                section_text, ["prerequisite", "requirement"]
            )
    ...
    def _create_mutation_from_dict(
        ...
        try:
            ...
            confidence = (
                float(confidence) / 100.0 if confidence > 1 else float(confidence)
            )
    ...
    def _extract_analysis_sections(self, text: str) -> Dict[str, Any]:
        ...
        section_patterns = {
            "bottlenecks": re.compile(
                r"(?:performance\s+)?bottlenecks?:?\s*(.*?)(?=\n\s*\w+:|$)",
                re.IGNORECASE | re.DOTALL,
            ),
            "opportunities": re.compile(
                r"optimization\s+opportunities?:?\s*(.*?)(?=\n\s*\w+:|$)",
                re.IGNORECASE | re.DOTALL,
            ),
