...
async def demonstrate_phase1_workflow():
    ...
    LeanRunner(
        lake_path=config.profiling.lake_path, lean_path=config.profiling.lean_path
    )

    rule_extractor = RuleExtractor()
    claude_client = ClaudeCodeClient(
        command=config.claude.command_path, timeout=config.claude.timeout
    )
...
"""
    ...
    try:
        ...
        print(
            f"  Success rate: "
            f"{simulated_profile_data['successful_rewrites']/(simulated_profile_data['successful_rewrites']+simulated_profile_data['failed_rewrites'])*100:.1f}%"
        )
...
def generate_simulated_claude_response() -> str:
    ...
    return """Based on the performance profile and simp rules analysis, """
    """here are my top optimization suggestions:
  ...
  "suggestions": [
    {
      ...
      "original_declaration": "@[simp ↓] theorem complex_rule (x y : Nat) [DecidableEq α] : "
      "(x + y).succ = x.succ + y",
      "mutated_declaration": "@[simp 50] theorem complex_rule (x y : Nat) [DecidableEq α] : "
      "(x + y).succ = x.succ + y",
      "reasoning": "This rule is only used 3 times but has high cost due to backward direction. "
      "Lowering priority and removing backward direction should improve performance.",
      "confidence": 85,
      "estimated_impact": {
    ...
    },
    {
      ...
      "reasoning": "This rule is used 15 times (most frequent). "
      "Higher priority will make simp try it earlier, potentially reducing search time.",
      "confidence": 78,
      "estimated_impact": {
    ...
    },
    {
      ...
      "reasoning": "Adding decidability condition can help simp make better decisions about when to apply this rule, "
...
The analysis shows that rule usage frequency and complexity are the main factors affecting performance. "
"The suggested mutations target these areas while maintaining correctness."""
