    ...
    try:
        ...
        if result.success and result.improvement_percent > 5.0:
            ...
            pr_url = await github_runner.create_optimization_pr(
                result, target_branch="simpulse/auto-optimization", source_branch="main"
            )
...
async def continuous_optimization_example():
    ...
    try:
        ...
        for trigger in triggers:
            logger.info(
                f"  - {trigger['trigger_id']}: {trigger['trigger_type']} "
...
async def metrics_and_reporting_example():
    ...
    config = Config(
        optimization=OptimizationConfig(
            population_size=15, generations=20, time_budget=1200, target_improvement=8.0
        ),
        claude=ClaudeConfig(backend="claude_code"),
        paths=PathConfig(
    ...
    metrics_collector = MetricsCollector(
        storage_dir=config.paths.output_dir / "metrics", enable_telemetry=True
    )
    ...
    try:
        ...
        dashboard_html = await report_generator.generate_dashboard(
            results=[result], title="Simpulse Optimization Dashboard"
        )
...
async def custom_configuration_example():
    ...
    try:
        ...
        if result.best_candidate:
            logger.info(
                f"🏆 Best candidate mutations: {len(result.best_candidate.mutations)}"
            )
            if result.best_candidate.fitness:
                logger.info(
                    f"📊 Best fitness score: "
                    f"{result.best_candidate.fitness.composite_score:.4f}"
                )
