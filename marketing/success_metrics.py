    ...
    def add_success(
        ...
        """,
            (
                ...
                record.rules_optimized,
                record.total_rules,
                record.build_time_before,
                record.build_time_after,
                record.user_name,
                record.testimonial,
            ),
        )
...
"""
    ...
    def get_impact_summary(self) -> ImpactSummary:
        ...
        co2_total = sum(self.calculate_co2_saved(r[4]) for r in self.get_all_records())
...
"""
    ...
    def _create_improvement_histogram(self, records: List[Tuple], output_dir: Path):
        ...
        plt.axvline(
            avg, color="red", linestyle="--", linewidth=2, label=f"Average: {avg:.1f}%"
        )
        plt.legend()
    ...
    def _create_impact_summary_graphic(self, summary: ImpactSummary, output_dir: Path):
        ...
        ax1.text(
            0.5,
            0.5,
            f"{summary.projects_helped}",
            fontsize=72,
            ha="center",
            va="center",
            color="#2ecc71",
        )
        ax1.text(0.5, 0.2, "Projects\nOptimized", fontsize=20, ha="center", va="center")
        ax1.set_xlim(0, 1)
        ax1.set_ylim(0, 1)
        ax1.axis("off")
        ...
        ax2.text(
            0.5,
            0.5,
            f"{summary.average_improvement:.0f}%",
            fontsize=72,
            ha="center",
            va="center",
            color="#3498db",
        ...
        ax2.text(
            0.5, 0.2, "Average\nImprovement", fontsize=20, ha="center", va="center"
        )
        ax2.set_xlim(0, 1)
        ax2.set_ylim(0, 1)
        ax2.axis("off")
        ...
        ax3.text(
            0.5,
            0.5,
            f"{summary.developer_hours_saved:.0f}",
            fontsize=48,
            ha="center",
            va="center",
            color="#e74c3c",
        ...
        ax3.text(
            0.5, 0.2, "Developer\nHours Saved", fontsize=20, ha="center", va="center"
        )
        ax3.set_xlim(0, 1)
        ax3.set_ylim(0, 1)
        ax3.axis("off")
        ...
        ax4.text(
            0.5,
            0.5,
            f"{summary.co2_saved_grams/1000:.1f}kg",
            fontsize=48,
            ha="center",
            va="center",
            color="#27ae60",
        )
        ax4.text(0.5, 0.2, "CO₂ Saved", fontsize=20, ha="center", va="center")
        ax4.set_xlim(0, 1)
        ax4.set_ylim(0, 1)
        ax4.axis("off")
