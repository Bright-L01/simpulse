...
class CodebaseCleaner:
    ...
    def _find_unused_imports(self) -> List[Tuple[str, str]]:
        ...
        for module, imports in self.all_imports.items():
            module_path = self.src_path / module.replace(".", "/").replace("/__init__", "") 
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if len(self.all_functions) > 50:
            recommendations.append("Consider splitting large modules into smaller, focused ones")
        
        if self.large_files:
            recommendations.append(f"Refactor large files: {[f[0] for f in self.large_files[:3]]}")
        
        if self.complex_functions:
            recommendations.append(f"Simplify complex functions: {[f[1] for f in self.complex_functions[:3]]}")
    ...
    def generate_cleanup_report(self) -> str:
        ...
        if report['duplicate_constants']:
            ...
            for const, modules in list(report['duplicate_constants'].items())[:5]:
                const_str = str(const)[:50] + "..." if len(str(const)) > 50 else str(const)
...
def remove_unused_functions():
    ...
    unused = %s
    print("Would remove %d unused functions" %% len(unused))
...
def remove_unused_imports():
    ...
    unused = %s
    print("Would remove %d unused imports" %% len(unused))

def extract_constants():
    """Extract duplicate constants to shared module."""
    duplicates = %s
    if duplicates:
        print("Would extract %d duplicate constants" %% len(duplicates))
...
''' % (
        analysis['unused_functions'][:10],
        analysis['unused_imports'][:10],
        list(analysis['duplicate_constants'].items())[:5]
    )
