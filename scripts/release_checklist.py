...
class ReleaseChecklist:
            ...
            if self.version:
                ...
                if tag_result.stdout.strip():
                    return CheckResult(
                        ...
                        fix_command="Remove existing tag or use a different version",
                    )
        ...
        try:
            ...
            if current_branch not in allowed_branches:
                return CheckResult(
                    ...
                    fix_command="Switch to main/master branch for release",
                )

            # Check if branch is up to date with remote
            subprocess.run(["git", "fetch"], cwd=self.project_root, capture_output=True)
    ...
    def check_documentation(self) -> CheckResult:
        ...
        if src_dir.exists():
            ...
            if files_without_docstring:
                issues.append(
                    f"{len(files_without_docstring)} files missing module docstrings"
                )
    ...
    def check_changelog(self) -> CheckResult:
        ...
        if (
            "unreleased" in content.lower()
            and len(content.split("unreleased")[-1].strip()) < 50
        ):
            return CheckResult(
                name="Changelog",
                status=CheckStatus.WARNING,
    ...
    def check_dependencies(self) -> CheckResult:
        ...
        if pyproject_path.exists():
            content = pyproject_path.read_text()
            if (
                "[tool.poetry.dependencies]" not in content
                and "[project.dependencies]" not in content
            ):
        ...
        for req_file in req_files:
            ...
            if req_path.exists():
                ...
                if unpinned:
                    issues.append(
                        f"{req_file} has {len(unpinned)} unpinned dependencies"
                    )
    ...
    def check_examples(self) -> CheckResult:
        ...
        for example in example_files[:5]:  # Check first 5
            try:
                result = subprocess.run(
                    ["python", "-m", "py_compile", str(example)], capture_output=True
                )
                if result.returncode != 0:
    ...
    def check_license(self) -> CheckResult:
        ...
        return CheckResult(
            name="License", status=CheckStatus.PASSED, message="LICENSE file exists"
        )
        ...
        for workflow in expected_workflows:
            ...
            if not workflow_path.exists():
                ...
                alternatives = [
                    ...
                    "test.yml" if workflow == "ci.yml" else workflow,
                    "publish.yml" if workflow == "release.yml" else workflow,
                ]
    ...
    def check_docker_build(self) -> CheckResult:
        ...
        try:
            ...
            # Clean up test image
            subprocess.run(
                ["docker", "rmi", "simpulse-test:latest"], capture_output=True
            )
            ...
            for status in CheckStatus:
                ...
                if status_checks:
                    ...
                    for check in status_checks:
                        ...
                        if check.details:
                            lines.append(
                                f"  - Details: {json.dumps(check.details, indent=2)}"
                            )
...
def main():
    ...
    parser.add_argument(
        "--version", type=str, help="Target release version (e.g., 1.0.0)"
    )
    parser.add_argument(
        "--report",
        type=Path,
