...
class ProductionAudit:
                ...
                if coverage_match:
                    ...
                    if coverage < 50:
                        self.issues["critical"].append(
                            f"Test coverage only {coverage}% (need 80%+)"
                        )
                    elif coverage < 80:
                        self.issues["warning"].append(
                            f"Test coverage {coverage}% (recommend 80%+)"
                        )
            ...
            if doc_path.exists():
                ...
                if doc == "README.md":
                    if "pip install simpulse" in content:
                        if (
                            "experimental" not in content.lower()
                            and "alpha" not in content.lower()
                        ):
                            self.issues["warning"].append(
                                "README should clearly state experimental status"
                            )

                    if "30%" in content and "simulation" not in content.lower():
                        self.issues["critical"].append(
                            "README claims 30% improvement without mentioning it's from simulation"
                        )

            else:
                docs_status[doc] = {"exists": False}
                self.issues["critical"].append(
                    f"Missing essential documentation: {doc}"
                )
    ...
    def security_audit(self) -> Dict[str, any]:
        ...
        for py_file in self.src_path.rglob("*.py"):
            ...
            for pattern, description in dangerous_patterns.items():
                if re.search(pattern, content, re.IGNORECASE):
                    ...
                    if (
                        "validate" not in py_file.name
                        and "security" not in py_file.name
                    ):
                        security_issues.append(f"{py_file.name}: {description}")
                        self.issues["critical"].append(
                            f"Security: {description} in {py_file.name}"
                        )
        ...
        return {"issues": security_issues, "secure": len(security_issues) == 0}
    ...
    def user_experience_test(self) -> Dict[str, any]:
        ...
        if readme.exists():
            ...
            if (
                "quick start" not in content.lower()
                and "getting started" not in content.lower()
            ):
        ...
        if cli_path.exists():
            ...
            if (
                "--help" not in content
                and "argparse" not in content
                and "click" not in content
            ):
        ...
        if error_count < 10:
            self.issues["warning"].append(
                "Very few error messages - may have poor error handling"
            )
    ...
    def check_code_quality(self) -> Dict[str, any]:
        ...
        if typing_ratio < 0.5:
            self.issues["warning"].append(
                f"Only {typing_ratio:.0%} of functions have type hints"
            )
        ...
        if req_file.exists():
            ...
            if content.strip():
                self.issues["info"].append(
                    "requirements.txt has dependencies - good for simple projects"
                )
    ...
    def verify_core_functionality(self) -> Dict[str, any]:
        ...
        core_modules = {
            ...
            "mutation_applicator": self.src_path
            / "evolution"
            / "mutation_applicator.py",
        }
        ...
        for name, path in core_modules.items():
            if path.exists():
                ...
                if not has_impl:
                    self.issues["critical"].append(
                        f"Core module {name} appears to be a stub"
                    )
            else:
        ...
        lean_integration = any(
            "lean" in str(f).lower() for f in self.src_path.rglob("*.py")
        )
