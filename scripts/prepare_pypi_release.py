...
class PyPIReleaser:
    ...
    def update_version(self) -> None:
        ...
        files_to_update = [
            ...
            (
                self.project_root / "src" / "simpulse" / "__init__.py",
                '__version__ = "{}"',
            ),
        ...
        for file_path, pattern in files_to_update:
            if file_path.exists():
                ...
                if pattern.format("") in content:
                    ...
                    current_version = re.search(pattern.format(r'([^"]+)'), content)
                    if current_version:
                        ...
                        logger.info(
                            f"Updated version in {file_path.name}: {old_version} → {self.config.version}"
                        )
    ...
    def build_distributions(self) -> bool:
        ...
        try:
            ...
            for dist_file in dist_files:
                logger.info(
                    f"✓ Built: {dist_file.name} ({dist_file.stat().st_size / 1024:.1f} KB)"
                )
...
"""
    ...
    def create_release_notes(self) -> None:
        ...
        if changelog_path.exists():
            ...
            pattern = (
                f"## \\[?{re.escape(self.config.version)}\\]?.*?\n(.*?)(?=## \\[|$)"
            )
    ...
    async def upload_to_pypi(self) -> bool:
        ...
        try:
            ...
            if self.config.test_pypi:
                logger.info(
                    f"View at: https://test.pypi.org/project/simpulse/{self.config.version}/"
                )
            else:
                logger.info(
                    f"View at: https://pypi.org/project/simpulse/{self.config.version}/"
                )
    ...
    def generate_release_checklist(self) -> None:
        ...
        lines = [
            ...
            "- [ ] Test installation: `pip install simpulse=={}`".format(
                self.config.version
            ),
            "- [ ] Create GitHub release",
            "- [ ] Tag the release: `git tag -a v{} -m 'Release v{}'`".format(
...
async def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Prepare Simpulse for PyPI release")
    parser.add_argument(
        "--version", type=str, required=True, help="Version to release (e.g., 1.0.0)"
    )
    parser.add_argument(
        "--test-pypi",
        action="store_true",
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Perform all steps except upload"
    )
    parser.add_argument(
        "--skip-tests", action="store_true", help="Skip running tests (not recommended)"
    )
    parser.add_argument(
        "--token", type=str, help="PyPI API token (or use PYPI_API_TOKEN env var)"
    )
    parser.add_argument(
