...
class Mathlib4Validator:
    ...
    async def setup_mathlib4(self) -> str:
        ...
        if self.mathlib_path.exists():
            ...
            subprocess.run(
                ["git", "fetch", "origin"], cwd=self.mathlib_path, check=True
            )
            subprocess.run(
        ...
        subprocess.run(["lake", "build"], cwd=self.mathlib_path, check=True)
...
"""
        ...
        for module in modules:
            ...
            try:
                ...
                results[module] = {
                    ...
                    "mutations": (
                        result.best_candidate.mutations if result.best_candidate else []
                    ),
                }

                logger.info(
                    f"  Applied {results[module]['mutations_applied']} mutations"
                )
                logger.info(
                    f"  Expected improvement: {result.improvement_percent:.1f}%"
                )
    ...
    def compile_validations(
        ...
        for module in modules:
            ...
            if base and opt:
                improvement = (
                    (base["total_time"] - opt["total_time"]) / base["total_time"]
                ) * 100
                ...
                validation = ModuleValidation(
                    ...
                    specific_improvements={
                        "simp_time": (
                            (base.get("simp_time", 0) - opt.get("simp_time", 0))
                            / base.get("simp_time", 1)
                        )
                        * 100
                    },
                )
    ...
    def generate_example_pr(
        self, optimizations: Dict[str, Any], validations: Dict[str, bool]
    ) -> str:
    ...
    def generate_report(self, results: Mathlib4Results) -> None:
        ...
        if results.total_improvement >= 20:
            ...
            lines.append(
                "⚠️ **Modest results.** May need tuning for mathlib4 specifics."
            )
...
async def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Validate Simpulse on mathlib4")
    parser.add_argument(
        "--workspace",
        type=Path,
    ...
    parser.add_argument(
        "--simpulse-root", type=Path, default=Path.cwd(), help="Simpulse project root"
    )
