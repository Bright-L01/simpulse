...
async def prove_simpulse_works() -> bool:
    ...
    if not check_lean_installation():
        ...
        logger.info(
            "2. Or run: curl https://raw.githubusercontent.com/leanprover/elan/master/elan-init.sh -sSf | sh"
        )
...
"""
        ...
        for i, mutation in enumerate(mutations):
            ...
            try:
                ...
                if time_ms < best_time:
                    ...
                    logger.info(
                        f"  ✓ Improvement: {time_ms:.2f}ms ({improvement:.1f}% faster)"
                    )
                else:
        ...
        if best_mutation:
            ...
            logger.info(
                "Simpulse can optimize simp rule priorities for real performance gains!"
            )
            return True
        else:
...
def run_simulated_proof() -> bool:
    ...
    logger.info(f"Best optimized time: {best_time:.2f}ms")
    logger.info(
        f"Total improvement: {(baseline_time - best_time) / baseline_time * 100:.1f}%"
    )
...
async def measure_simp_performance(project_dir: Path) -> float:
    ...
    profile_cmd = ["lake", "env", "lean", "--profile", "TestProject.lean"]
    ...
    for _ in range(3):
        ...
        result = subprocess.run(
            profile_cmd, cwd=project_dir, capture_output=True, text=True
        )
    ...
    try:
        ...
        if mutation.rule_name == "multi":
            ...
            if mutation.new_annotation:
                # Replace annotation
                pattern = (
                    f"{mutation.old_annotation}\\s+theorem\\s+{mutation.rule_name}"
                )
...
def revert_mutation(lean_file: Path, mutation: SimpleMutation) -> bool:
    ...
    try:
        ...
        if mutation.rule_name == "multi":
            ...
            if mutation.new_annotation:
                pattern = (
                    f"{mutation.new_annotation}\\s+theorem\\s+{mutation.rule_name}"
                )
