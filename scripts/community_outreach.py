...
class ProjectFinder:
    ...
    def _github_search(self, query: str) -> List[Dict]:
        ...
        try:
            result = subprocess.run(
                ["gh", "search", "repos", query, "--limit", "30", "--json", 
    ...
    def _normalize_repo(self, repo: Dict) -> Dict:
        ...
        return {
            ...
            'stargazers_count': repo.get('stargazersCount', repo.get('stargazers_count', 0)),
    ...
    def _calculate_potential(self, repo: Dict, perf_issues: List[str], simp_count: int) -> float:
    ...
    def generate_outreach_materials(self, candidates: List[ProjectCandidate]) -> Dict[str, str]:
...
"""
    ...
    def _generate_announcement(self, candidates: List[ProjectCandidate]) -> str:
        ...
        avg_potential = sum(c.optimization_potential for c in candidates) / len(candidates)
...
"""
    ...
    def _generate_candidate_summary(self, candidates: List[ProjectCandidate]) -> str:
        ...
        return f"""# Lean Project Optimization Candidates
...
""" for c in candidates if c.optimization_potential > 70) + """

## Medium Potential Projects (50-70)

""" + "\n".join(f"- [{c.name}]({c.url}): {c.optimization_potential:.0f}/100 potential\n" 
                for c in candidates if 50 <= c.optimization_potential <= 70) + """
...
async def main():
    ...
    if candidates:
        ...
        for i, c in enumerate(candidates[:5], 1):
            print(f"{i}. {c.name}: {c.optimization_potential:.0f}/100 potential, "
