...
class ComprehensiveTester:
    ...
    def _analyze_file_gaps(self, file_path: Path, missing_lines: List[int]) -> List[CoverageGap]:
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef):
                    ...
                    missing_in_func = [l for l in missing_lines if l in func_lines]
...
'''
            ...
            if perf_file.exists():
                ...
                if baseline_file.exists():
                    ...
                    for result in perf_data.get("results", []):
                        ...
                        if baseline_result:
                            if result["execution_time"] > baseline_result["execution_time"] * 1.1:
                                regressions.append({
                                    "name": result["name"],
                                    "regression": (result["execution_time"] / baseline_result["execution_time"] - 1) * 100
                                })
    ...
    def generate_report(self, results: Dict[str, Any]) -> None:
        ...
        unit_results = results["test_results"].get("unit", TestResult("unit", 0, 0, 0, 0, 0, [], {}))
        ...
        if "gaps" in results.get("coverage_analysis", {}):
            ...
            for gap in gaps[:5]:
                lines.append(f"- `{gap.module}.{gap.function}` - Lines: {gap.lines[:5]}...")
...
async def main():
    ...
    if current_coverage < args.target_coverage:
        logger.warning(f"Coverage {current_coverage:.1f}% is below target {args.target_coverage}%")
    ...
    logger.info(f"Report: test_report.md")
    logger.info("="*60)
