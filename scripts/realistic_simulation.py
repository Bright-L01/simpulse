...
def simulate_simp_execution(rules: List[SimpRule], iterations: int = 1000) -> float:
    ...
    for _ in range(iterations):
        ...
        for rule in sorted(
            rules, key=lambda r: (r.priority != "high", r.priority == "low")
        ):
...
def run_realistic_simulation():
    ...
    for rule in rules:
        print(
            f"  @[simp] theorem {rule.name} (priority: {rule.priority}, "
    ...
    print(
        "  - High-frequency simple rules (add_zero, mul_one) checked after complex ones"
    )
    ...
    for rule in optimized_rules_1:
        if (
            rule.name in ["add_zero", "mul_one", "zero_add"]
            and rule.usage_frequency > 60
        ):
            rule.priority = "high"
    ...
    print(
        f"  Result: {combined_time*1000:.2f}ms ({combined_improvement:.1f}% improvement)"
    )
    ...
    for opt in optimizations:
        print(
            f"{opt.mutation:<50} {opt.new_time*1000:<10.2f} "
    ...
    print(
        f"  Simp tactic time: {module_build_time * simp_percent:.1f}s ({simp_percent*100:.0f}%)"
    )
    print(f"  With {combined_improvement:.1f}% simp improvement:")
    print(
        f"    New simp time: {module_build_time * simp_percent * (1 - combined_improvement/100):.1f}s"
    )
    print(
        f"    Total time saved: {module_build_time * simp_percent * combined_improvement/100:.1f}s"
    )
    print(
        f"    New total build time: {module_build_time - (module_build_time * simp_percent * combined_improvement/100):.1f}s"
    )
    print(
        f"    Overall improvement: {(module_build_time * simp_percent * combined_improvement/100) / module_build_time * 100:.1f}%"
    )
