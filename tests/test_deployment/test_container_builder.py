...
class TestContainerBuilder:
    ...
    def builder(self):
        ...
        return ContainerBuilder(
            base_image="leanprover/lean4:latest",
            push_registry=None
        )
...
""")
    ...
    def test_create_deployment_package(self, builder, temp_dir):
        ...
        (project_path / "src").mkdir()
        (project_path / "src" / "Main.lean").write_text("def main : IO Unit := IO.println \"Hello\"")
        
        # Create deployment package
        package_path = builder.create_deployment_package(
            project_path=project_path,
            include_compose=True,
            include_github_action=True
        )
...
"""
    ...
    def test_validate_docker_available(self, builder):
        ...
        with patch('subprocess.run') as mock_run:
            ...
            assert builder._validate_docker_available() == True
            
            # Docker not available
            mock_run.side_effect = FileNotFoundError()
            assert builder._validate_docker_available() == False
...
""")
        ...
        if builder._validate_docker_available():
            result = builder.build_image(
                context_path=temp_dir,
                tag="simpulse-integration-test:latest"
            )
            
            # Cleanup
            import subprocess
            subprocess.run(["docker", "rmi", "simpulse-integration-test:latest"],
                         capture_output=True)
