...
from simpulse.evolution.models import (ModuleRules, MutationSuggestion,
                                       MutationType, OptimizationGoal,
                                       SimpDirection, SimpPriority, SimpRule,
                                       SourceLocation)
from simpulse.evolution.models_v2 import (Candidate, EvolutionConfig,
                                          EvolutionHistory, FitnessScore,
                                          GenerationResult, Mutation,
                                          Population)
...
class TestCandidate:
    ...
    def test_candidate_comparison(self):
        ...
        candidate2 = Candidate(
            mutations=[],
            fitness=FitnessScore(composite_score=0.6)
        )
...
class TestGenerationResult:
    ...
    def test_generation_result_creation(self):
        ...
        assert result.new_best_found == True
...
class TestEvolutionConfig:
    ...
    def test_evolution_config_validation(self):
        ...
        invalid_config = EvolutionConfig(
            mutation_rate=1.5,
            crossover_rate=0.8
        )
...
class TestModelsIntegration:
    ...
    def test_complete_evolution_workflow(self):
        ...
        for i in range(10):
            candidate = Candidate(
                mutations=[],
                fitness=FitnessScore(composite_score=0.5 + i * 0.05)
            )
        ...
        assert history.total_generations == 5
        assert history.generations[-1].best_fitness > history.generations[0].best_fitness