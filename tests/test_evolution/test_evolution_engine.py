...
class TestEvolutionEngine:
    ...
    def evolution_engine(self, mock_config, mock_claude_client):
        ...
        with patch(
            "simpulse.evolution.evolution_engine.ClaudeCodeClient",
    ...
    @pytest.mark.asyncio
    async def test_run_evolution_basic(
        self, evolution_engine, mock_lean_project, sample_simp_rules
    ):
        """Test basic evolution run."""
        # Mock components
        with patch.object(
            evolution_engine.rule_extractor,
            "extract_rules",
            return_value=sample_simp_rules,
        ):
            with patch.object(
                evolution_engine.population_manager, "create_initial_population"
            ) as mock_create_pop:
                with patch.object(
                    evolution_engine.fitness_evaluator, "evaluate_candidate"
                ) as mock_evaluate:
                    # Setup mocks
                    mock_population = Population(
                        candidates=[Candidate(mutations=[]), Candidate(mutations=[])]
                    )
    ...
    async def test_run_single_generation(self, evolution_engine, sample_simp_rules):
        ...
        with patch.object(
            evolution_engine.population_manager, "select_parents"
        ) as mock_select:
            with patch.object(
                evolution_engine.population_manager, "crossover"
            ) as mock_crossover:
                with patch.object(evolution_engine, "_mutate_candidate") as mock_mutate:
                    with patch.object(
                        ...
                        mock_select.return_value = [
                            test_population.candidates[0],
                            test_population.candidates[1],
                        ]
                        ...
                        # Run generation
                        result = await evolution_engine._run_generation(
                            test_population, sample_simp_rules
                        )
    ...
    @pytest.mark.asyncio
    async def test_mutation_with_claude(
        self, evolution_engine, sample_simp_rules, mock_claude_client
    ):
...
2. Change direction of mul_zero to pre-simplification""",
        ...
        with patch.object(evolution_engine, "_parse_claude_suggestions") as mock_parse:
            ...
            mutated = await evolution_engine._mutate_candidate(
                candidate, sample_simp_rules
            )
    ...
    async def test_performance_tracking(self, evolution_engine):
        ...
        with patch.object(
            evolution_engine.lean_runner, "profile_module"
        ) as mock_profile:
            ...
            baseline = await evolution_engine._establish_baseline(
                ["TestModule"], Path(".")
            )
...
3. Disable rule_three temporarily"""
        ...
        suggestions = evolution_engine._parse_claude_suggestions(
            claude_response, [SimpRule(rule_name="rule_one", full_attribute="@[simp]")]
        )
    ...
    async def test_concurrent_evaluation(self, evolution_engine):
        ...
        with patch.object(
            ...
            tasks = [
                evolution_engine.fitness_evaluator.evaluate_candidate(c, {})
                for c in candidates
            ]
