...
def not_simp_attr : Nat → Nat := id
        ...
        for i, test_case in enumerate(edge_cases):
            ...
            # Count expected
            expected = len([line for line in test_case.split('\n') 
                          if line.strip().startswith('@[simp') and 
    ...
    def _save_results(self):
        ...
        result_data = {
            ...
            "summary": {
                ...
                "total_actual_rules": sum(r["actual_simp_count"] for r in self.results if r["success"]),
                "total_extracted_rules": sum(r["extracted_count"] for r in self.results if r["success"]),
                "overall_accuracy": round(
                    sum(r["extracted_count"] for r in self.results if r["success"]) / 
                    sum(r["actual_simp_count"] for r in self.results if r["success"]) * 100, 2
