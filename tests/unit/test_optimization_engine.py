...
class TestOptimizationEngine:
    ...
    def test_generate_priority_increase_recommendations(self, engine, sample_diagnostic_analysis):
        ...
        priority_increase_recs = [
            r for r in recommendations if r.optimization_type == OptimizationType.PRIORITY_INCREASE
        ]
        ...
        high_usage_rec = next(
            (r for r in priority_increase_recs if r.theorem_name == "high_usage_theorem"), None
        )
    ...
    def test_generate_priority_decrease_recommendations(self, engine, sample_diagnostic_analysis):
        ...
        priority_decrease_recs = [
            r for r in recommendations if r.optimization_type == OptimizationType.PRIORITY_DECREASE
        ]
        ...
        low_success_rec = next(
            (r for r in priority_decrease_recs if r.theorem_name == "low_success_theorem"), None
        )
    ...
    def test_generate_loop_detection_recommendations(self, engine, sample_diagnostic_analysis):
        ...
        loop_detection_recs = [
            r for r in recommendations if r.optimization_type == OptimizationType.LOOP_DETECTION
        ]

        # Check if excessive usage is detected
        excessive_usage_rec = next(
            (r for r in loop_detection_recs if r.theorem_name == "excessive_usage_theorem"), None
        )
    ...
    def test_generate_inefficient_removal_recommendations(self, engine, sample_diagnostic_analysis):
        ...
        unused_rec = next(
            (r for r in inefficient_removal_recs if r.theorem_name == "unused_theorem"), None
        )

        if unused_rec:
            assert unused_rec.confidence > 0
            assert (
                "unused" in unused_rec.reason.lower() or "never used" in unused_rec.reason.lower()
            )
    ...
    def test_calculate_confidence_priority_increase(self, engine):
        ...
        theorem_usage = SimpTheoremUsage(
            name="test_theorem", used_count=100, tried_count=110, success_rate=0.909
        )

        confidence = engine._calculate_confidence(theorem_usage, OptimizationType.PRIORITY_INCREASE)
    ...
    def test_calculate_confidence_priority_decrease(self, engine):
        ...
        theorem_usage = SimpTheoremUsage(
            name="test_theorem", used_count=20, tried_count=100, success_rate=0.2
        )

        confidence = engine._calculate_confidence(theorem_usage, OptimizationType.PRIORITY_DECREASE)
    ...
    def test_calculate_confidence_loop_detection(self, engine):
        ...
        theorem_usage = SimpTheoremUsage(
            name="test_theorem", used_count=1000, tried_count=1010, success_rate=0.99
        )

        confidence = engine._calculate_confidence(theorem_usage, OptimizationType.LOOP_DETECTION)
    ...
    def test_calculate_confidence_inefficient_removal(self, engine):
        ...
        theorem_usage = SimpTheoremUsage(
            name="test_theorem", used_count=0, tried_count=50, success_rate=0.0
        )

        confidence = engine._calculate_confidence(
            theorem_usage, OptimizationType.INEFFICIENT_REMOVAL
        )
    ...
    def test_suggest_priority_increase(self, engine):
        ...
        new_priority = engine._suggest_priority(theorem_usage, OptimizationType.PRIORITY_INCREASE)
    ...
    def test_suggest_priority_decrease(self, engine):
        ...
        new_priority = engine._suggest_priority(theorem_usage, OptimizationType.PRIORITY_DECREASE)
    ...
    def test_generate_reason_priority_increase(self, engine):
        ...
        reason = engine._generate_reason(theorem_usage, OptimizationType.PRIORITY_INCREASE)
    ...
    def test_generate_reason_priority_decrease(self, engine):
        ...
        reason = engine._generate_reason(theorem_usage, OptimizationType.PRIORITY_DECREASE)
    ...
    def test_filter_recommendations_by_confidence(self, engine, sample_diagnostic_analysis):
        ...
        high_confidence_recs = engine.filter_recommendations_by_confidence(
            all_recommendations, confidence_threshold=80.0
        )
    ...
    def test_filter_recommendations_by_type(self, engine, sample_diagnostic_analysis):
        ...
        priority_increase_recs = engine.filter_recommendations_by_type(
            all_recommendations, OptimizationType.PRIORITY_INCREASE
        )
    ...
    def test_recommendation_validation(self, engine):
        ...
        analysis = DiagnosticAnalysis(
            total_theorems=1, total_files=1, theorem_usage=[theorem_usage]
        )
