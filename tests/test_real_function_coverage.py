...
from simpulse.evolution.models import Mutation, OptimizationCandidate, SimplificationRule
...
class TestErrorHandlerRealFunctions:
    ...
    def test_generate_recovery_suggestions_all_categories(self):
        ...
        test_cases = [
            ...
            (
                ErrorCategory.LEAN_EXECUTION,
                Exception("lean failed"),
                ["Lean 4 is installed", "lake build"],
            ),
            (
                ErrorCategory.OPTIMIZATION,
                Exception("opt failed"),
                ["conservative optimization", "reduce the number"],
            ),
            (
                ErrorCategory.VALIDATION,
                Exception("validation failed"),
                ["compile correctly", "backup files"],
            ),
            (
                ErrorCategory.CONFIGURATION,
                Exception("config error"),
                ["configuration file syntax", "required configuration"],
            ),
            (
                ErrorCategory.DEPENDENCY,
                Exception("dep error"),
                ["missing dependencies", "Python version"],
            ),
        ]
        ...
        for category, exception, expected_terms in test_cases:
            ...
            assert (
                found
            ), f"Expected one of {expected_terms} in suggestions for {category}: {suggestions}"
    ...
    def test_generate_recovery_suggestions_exception_specific(self):
        ...
        for exception, expected_text in test_exceptions:
            ...
            assert (
                found
            ), f"Expected '{expected_text}' in suggestions for {exception}: {suggestions}"
...
class TestEvolutionModelsRealFunctions:
    ...
    def test_simplification_rule_creation(self):
        ...
        rule = SimplificationRule(
            name="test_rule", lhs="x + 0", rhs="x", conditions=["x : ℕ"], priority=100
        )
    ...
    def test_mutation_creation(self):
        ...
        mutation = Mutation(
            type="priority_change", target_rule=rule, old_value=100, new_value=50, impact_score=0.8
        )
    ...
    def test_optimization_candidate_creation(self):
        ...
        candidate = OptimizationCandidate(
            mutations=[mutation1, mutation2], expected_improvement=0.15, confidence=0.85
        )
